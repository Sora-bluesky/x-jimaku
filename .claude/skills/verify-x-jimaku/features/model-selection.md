...
- **Recommendation hint.** After the page's WebGPU probe resolves, check the `#model-recommendation` element and confirm it matches `recommendModel`'s rule for the actual environment: `tiny` when running on WASM or without adapter info, `small` on WebGPU with a recent NVIDIA architecture, hidden otherwise. Screenshot it alongside the picker.
