...
ビルドの `dirty` 判定は `git status --porcelain` が空かどうかだけで、**拡張のバンドルに関係のないファイルでも成立します**（`vite.config.ts` の `getGitBuildState`）。

2026-09-04 時点で汚しているのは次の 2 群で、どちらも拡張の中身とは無関係です。

- `.claude/skills/verify-x-jimaku/` の 6 ファイル（別セッションが編集中）
- `.claude/launch.json`（未追跡）

このまま `npm run zip` を実行すると拒否されます。拒否は正しい振る舞いですが、**自分の作業を全部コミットしても解消しない**ので、初めて見ると原因を探すことになります。

tag を打つ前に、別セッション側を含めてツリーを空にしてください。`--allow-dirty` で通すのは開発中の試行だけで、配布する zip には使いません。
