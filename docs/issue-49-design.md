...
Status: rev4・凍結（2 巡目の残 2 件 = 引数検証の明記・基準値 14 件への再固定と正規表現表記の修復を反映。基準値は host が再検証済み）
Date: 2026-08-30
...
**ゴール（rev4・2026-08-30 sora 裁定 = 誤義撲滅主義）**: リストにある固有名詞が**誤義**
（別の語・地名として解釈される。例: Roman → 都市のローマ）にならないこと。片仮名の音写
（ローマン/ロマン等）は許容し、原綴り率は記録指標に降格する。必須の機械ゲート =
`ローマ(?!ン)` 一致 0 件。judge の誤訳は改訂 ref（片仮名許容の判定注記つき）で before を
再採点した値と比較する。
