...
## rev8: masking 方式（rev7 再レビューの残 1 件 = 照合・境界規則の明文化を反映・凍結）

### 原理

翻訳前に、節の EN テキスト中の contextTerms を不透明プレースホルダに置換して翻訳し、
出力のプレースホルダを原綴りで復元する。固有名詞が翻訳器を**通らない**ため、誤義は
構造的に発生しない。プロンプトレバーと違いモデルの従順さに依存しない。

### プレースホルダ生存プローブ（2026-08-30・実 Chrome ページ JS で実測）

| 候補 | Translator API | LanguageModel (Nano) | 判定 |
|---|---|---|---|
| `%%N%%` | 生存 | **8/9 生存** | **採用** |
| `⟦N⟧` | 生存 | 0/3（閉括弧が全角 ］ に化ける） | 不採用 |
| `〖N〗` | 生存 | 0/3（丸ごと消える） | 不採用 |
| `@@N@@` / `XQZN` | 生存 | 未測 / XQZ は訳質低下の副作用 | 予備 |

Nano の非生存 1 回は「プレースホルダを無視して自由生成」（全欠落）。→ 単独では 100% に
ならないため、下記の救済梯子で吸収する。

### 設計（rev7・一対一対応の原則で再構成）

1. **maskPlan**: 節テキスト中の `activeContextTerms` を長い順・**文字列一致
   （regex エスケープ済み・大小文字区別）**で走査し、**出現ごとに一意な連番** `%%N%%` を
   割り当てる（同語 2 回出現なら %%1%% と %%2%%）。plan = 順序付き {N → 原綴り} +
   期待復元数。**出現が上限 4 を超える節は mask せず原文素通し**（安全側 = 部分 mask で
   保証が破れる状態を作らない）。マッチなしなら mask も restore もしない（現行同一経路）。
   **出現の境界規則**: term の直前が `[A-Za-z0-9]` でなく、直後が `[A-Za-z0-9']` でない
   ときだけ一致とする（Roman's / Romans / 小文字化出現は丸ごと非対象 = 部分 mask を
   作らない）。
2. **共通梯子への統合（新設ではなく一般化）**: 既存の `rescueLanguageModelLine`
   （Translator 2 段 + 英語素通しを内包・`translate.ts:644-722`）を
   `{original, masked, maskPlan}` を受ける**行単位の共通梯子**へ一般化する。
   Nano 主経路・Translator 主経路のどちらでも、翻訳呼び出しは masked テキストを使い、
   出力は同一の restore + 欠落判定を通る。並行する第 2 の梯子は作らない。
3. **restore と検査の順序（確定）**: (a) 出力に `%%\s*N\s*%%` の復元を適用 →
   (b) **期待した各番号がちょうど 1 回ずつ出現し、未知番号・重複・欠落・未解決 `%%` が
   いずれも 0** を検査（1 つでも破れたら梯子を降下）→
   (c) `isBadLanguageModelResponse` は**復元後テキスト**で評価。長さ比検査の分母は
   **mask 前の原文**、`removeProperNouns` には全 terms を渡す（現行同様）。
   recentPairs には復元後ペアを記録（呼び出し順は現状のまま成立）。
4. **プロンプト側の整合（「変更なし」を撤回）**: 固有名詞ブロックと recentPairs が
   原綴りをそのまま描画すると mask した語が Nano へ漏れる（`translate.ts:1132-1159`）。
   現在節で mask した語は**固有名詞ブロックから除外**し、recentPairs 内の同語も
   **同じ plan で再 mask** して描画する。不良応答検査用の全 terms リストは削らない。
5. **衝突**: 原文に literal な `%%N%%` が含まれる場合は誤復元し得る（受容限界・テストで
   挙動固定）。所有格・複数形（Roman's / Romans）は**一致しない**＝mask 対象外
   （原文素通しで誤義リスクは残るが保証は破れない。受入例に明記）。ASR が小文字化した
   出現も対象外（大小文字区別の方針を受入例で固定）。

### 受け入れ（rev7・一対一対応で再定義）

1. 単体テスト: 出現ごと一意番号（同語 2 出現）・長い順優先（Kennedy Space Center >
   Kennedy）・regex メタ文字 term（U.S.）・上限超過で原文素通し・空白ゆらぎ復元・
   期待数不一致で梯子降下・Translator 主経路でも同じ restore/判定・literal `%%N%%` 衝突・
   Roman's / Romans / 小文字出現が対象外である挙動・固有名詞ブロックからの除外と
   recentPairs 再 mask の描画。
2. live2 after（同一手順・prompt-api/base）: **必須ゲート = `ローマ(?!ン)` == 0
   （誤義・before 14）かつ 出力中の未解決 `%%` == 0**。`(?<![A-Za-z])Roman(?![A-Za-z])`
   は**記録指標**（閾値なし。ループ回数と節欠落に依存するため）。judge 誤訳が改訂 ref
   before（5）+ ノイズ 2 以内。
3. tts 側 after 1 回（誤訳 before(10)+2 以内）。
4. CfT WER: tts/base ×2 が帯内。
