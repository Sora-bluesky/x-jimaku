...
- 同じ名前に 2 通り以上の表記が出た＝**揺れている**、の検出と、候補ごとの頻度・出た走行・経路。
  共起の判定は多数決で行います（実装順 2 の実走で決めた規則・2026-09-06）: 英語側が分かる単位だけを数え、その名前を
  含まない単位に出た回数が含む単位に出た回数より多く、かつ両方の和が 3 以上のときだけ候補から外します。
  1 単位の ASR の聞き違い（"NASA gottered"）で「ゴッダード」が Goddard の票から消えたのが理由です。英語側が
  無い単位（旧形式の走行）は「その名前を含まない単位」に数えません。別の名前に**確定で**帰属した形
  （`expected` / `keptLatin` / `wrongKnown`）だけを他の名前の候補から外し、`variant` は外しません
  （同じカタカナ連は両方の名前と共起している）。入れ子の名前（Goddard と NASA Goddard）は互いに外しません。
...
モードでページ由来になる語（§4-2 の規約でページ由来になる綴りだけが `terms.txt` にある）: **原綴り**。ただし
ページ由来の語が定訳の行を内側に含む（`NASA Goddard` と `Goddard` = ゴダード）ときは、その部分を `ja` に置き換えた形
（`NASA ゴダード`。ラテン語と日本語の境の空白は有無を問わない）が `expected` です。表がページより先にマスクを取る
（§2-5）ので、復元はその形で出ます。2026-09-06 の before/after で `NASA ゴダード` が `wrongKnown` に落ちたので追記。表が
与える形ではなく、ページ由来のマスクが原綴りで復元する（§1-3）という、§2-4 が「変えない」と言う既定そのものです。
`expected` は**名前ごと**に 1 つで、出現ごとには決めません。マスクは節ごとに決まる（ページ由来の出現が節に 5 つ以上
あれば計画自体が無い・`term-masking.ts:38-46`）ので、同じ名前が守られた出現と守られなかった出現を 1 走行に持ちます。
守られなかった出現にも名前の `expected` を当てるのは意図で、A は同じクリップの before と after を比べる（§5-3）ため、
枠の奪い合いで守られなくなった出現は `variant` / `missing` に落ちて A-4 の率を下げ、それが捕まえたい後退です
（下の A-4）。出現ごとに計画を見て `expected` を外すと、守れなくなったことが率から消えます。その代わり
`nameExpectedRate` の絶対値は翻訳の良さではなく、同じクリップの比較にしか読みません（分母を併記する規約と同じ）。
...
曖昧語は節に版や姉妹名の証拠があればその出現だけ守られます（`glossary.ts:77-90`）が、それは節の文面が決める形で
名前の形ではないので、A には入れません。そこでの後退は `nameVariants` の観測にしか出ず、A は見逃します。
見逃す側に倒すのは、証拠付きの出現だけを拾って名前の率にすると、証拠の有無が走行ごとに違う ASR の揺れが
率の差に化けるからです。
...
切った後のものです。①ページの `sources[].id` はすべて `lines[]` にあり、同じ id の `text` が一致する。ただし、`sources[].id` が
**すべて** `lines[]` に無く、かつ**すべて** `lines[]` の最小 id より小さいページは、切りの前に受理された行の cue が
再生開始後まで表示に残っていたもの（ウォームアップの尾。2026-09-05 の実走行 4 本中 3 本で id 3〜7 がこれに当たった）
なので、単位に入れず `warmupPagesExcluded` に数えて①の対象から外す。一部の id だけ無い、または無い id が最小 id 以上の
ページは①の破れとして扱う ②`drops[]` の
...
裁定が要った 4 点（本文は選んだ案で書いてあります）。**1 番は 2026-09-04、3 番と 4 番は 2026-09-05 に sora が本文の案で裁定し、2 番は 1 番に従属して確定しました。** 以下は裁定時の選択肢の記録です:
