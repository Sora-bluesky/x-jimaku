...
| `observations.katakanaNameHits` | 日本語出力における既知のカタカナ音訳（オプス・オパウス・オピュス・クロード・アンソロピック・ハルキング・ゴダード・クラーク）の出現回数。下がるべき値。合否には使わない。内訳は次の2値の合計。 |
| `observations.katakanaNameHitsAmbiguous` | そのうち曖昧語の音訳（オプス・オパウス・オピュス・クラーク）。この変更が証拠付きで止めるべき漏れ。合否には使わない。 |
| `observations.katakanaNameHitsPlain` | そのうち非曖昧語の音訳（クロード・アンソロピック・ハルキング・ゴダード）。マスク穴。合否には使わない。 |
